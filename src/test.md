...
1. FEアプリ構成設計(RBSC・遅延ロード)
   - 担当: FE-L
   - 内容: Spartacus の機能モジュール構成、遅延ロードの単位、共通部品の配置方針。あわせて RBSC（SAP のプライベート npm レジストリ）から Composable Storefront ライブラリを取得する設定を確立する — .npmrc のレジストリ URL とアクセストークンの管理方法（Git に置かない）、ローカル／CI／CCv2 ビルド（js-storefront/manifest.json の enabledRepositories: ["spartacus-6"]）の 3 経路で同じ版が解決されることの確認、yarn.lock の管理。
...
9. UAT支援(シナリオ・データ・指摘対応)
   - 担当: PM / FE×2 / BE×2 / 業務
   - 内容: 業務部門が行う受入テストのシナリオ作成支援、テストデータ準備、環境の払い出し、指摘の切り分けと修正。合否判定は業務部門。

10. 障害対応訓練
    - 担当: PM / BE-L / FE-L / BE×1 / FE×1
    - 内容: A-3-39 の訓練シナリオを実施。障害の検知 → OpenSearch / Dynatrace での切り分け → SAP for Me 起票 → 復旧 → 報告までを通しで演習し、Runbook の不備を洗い出す。

11. 運用受入テスト
    - 担当: PM / BE-L / 運用チーム
    - 内容: 運用チームが Runbook・監視・アラート・バックアップ／リストア・デプロイ手順を実際に操作し、引き継ぎ可能な状態かを確認。不足は A-3 の運用設計へ差し戻す。

12. 編集者研修
    - 担当: BE×1 / FE×1 / 業務
    - 内容: CMS 編集者（業務部門）向けに SmartEdit / Backoffice でのページ編集・コンポーネント配置・Staged→Online 同期の手順を研修。Spartacus で編集できる範囲（A-3-8 の allowlist）の説明を含む。

13. 切替練習
    - 担当: PM / BE-L / FE-L
    - 内容: 本番切替の手順（DNS / エンドポイントのドメイン変更・IP フィルタ解放・旧 Storefront の扱い・ロールバック）を staging でリハーサル。所要時間を実測し、切替当日の分単位の手順書に反映。

14. 移行の判定
    - 担当: PM（判定は ステコミ / 業務）
    - 内容: A-2-5 の品質ゲート（テスト結果・性能・セキュリティ診断・CRC 結果・UAT 合否）を揃え、Go / No-Go を判定する会議の運営と判定資料の作成。

## A-6 移行・運用移管

1. 段階公開
   - 担当: PM / BE-L / FE-L
   - 内容: 一部ユーザー・一部トラフィックから新ストアフロントを公開し、段階的に比率を上げる。CCv2 の Blue/Green・canary（Storefront / JS Storefront / API で利用可）を使う場合は A-4-33 の判定基準書に従う。

2. 切替効果測定・新旧分析比較
   - 担当: PM / FE-L
   - 内容: A-1-8 の現状値と A-3-12 の KPI を使い、新旧のコンバージョン・応答時間・エラー率・離脱率を比較。効果と劣化箇所を報告。

3. ハイパーケア
   - 担当: 全員（当番制）
   - 内容: 公開直後の一定期間（通常 2〜4 週間）、開発チームが優先度を上げて障害・問い合わせに即応する体制。対応記録は Runbook と troubleshooting に反映。

4. 並行稼働保守
   - 担当: BE×2 / FE×1
   - 内容: 旧 Accelerator（accstorefront）と新 Spartacus を並行稼働させる期間の保守。両方に影響する BE 変更（OCC・CMS データ）の同時反映と、旧側のセキュリティパッチ適用。

5. WAFルールのチューニング
   - 担当: BE-L
   - 内容: 本番トラフィックを見ながら Cloud Portal の WAF・レート制限・bot 対策ルールの誤検知（正規ユーザーのブロック）と検知漏れを調整。

6. 環境管理・データリフレッシュの定常運用化
   - 担当: BE-L / BE×1 / 運用チーム
   - 内容: dev / staging への本番データのリフレッシュ手順（匿名化を含む）、環境ごとのプロパティ管理、不要ビルドの整理を定常業務として手順化。

7. 旧資産の停止・撤去
   - 担当: BE-L / BE×2 / PM
   - 内容: 並行稼働終了後、manifest から accstorefront aspect と storefrontAddons を外し、JSP・Controller・AddOn・不要な CMS 型を削除。Storefront エンドポイントの停止、旧 URL のリダイレクト維持期間の決定。

8. 運用チームへの引き継ぎ
   - 担当: PM / BE-L / FE-L / 運用チーム
   - 内容: Runbook・監視・アカウント・SAP チケット起票・パッチ追従サイクル・RBSC トークン更新を運用チームへ移管。A-5-11 の運用受入テストで出た不足の解消を完了条件にする。
...
- **RBSC（Repository-Based Shipment Channel）**: SAP がライセンス製品のライブラリを配布するプライベート npm レジストリ。Composable Storefront 5.0 以降のライブラリは公開 npm には無く、ここから取得する（リポジトリ番号 73554900100900004337）。SAP Repositories Management で発行したアクセストークンを .npmrc に設定して npm / yarn からアクセスする。CCv2 のビルド側では js-storefront/manifest.json の enabledRepositories に "spartacus-6" を指定することで同じライブラリが解決される。トークンは秘密情報として扱い、Git にコミットしない。
...
- **UAT（User Acceptance Test）**: 業務部門が実業務の観点で行う受入テスト。合否判定は業務部門が持つ。
- **運用受入テスト**: 運用チームが、引き継がれる手順・監視・ツールで実際に運用できるかを確認するテスト。
- **切替練習（リハーサル）**: 本番切替の手順を staging で通しで実施し、所要時間と手順の抜けを確認すること。
- **Go / No-Go 判定**: 品質ゲートの結果を揃えて、本番切替を実施するか延期するかを決める意思決定。
- **段階公開**: 一部のユーザー・トラフィックから新システムを公開し、比率を段階的に上げる方式。
- **ハイパーケア**: 本番公開直後に開発チームが優先対応する集中サポート期間。
- **並行稼働**: 新旧システムを同時に稼働させる期間。BE 変更を両方に反映する負荷が発生する。
- **データリフレッシュ**: 本番データを下位環境（dev / staging）へコピーして最新化すること。個人情報は匿名化する。
- **運用チーム**: 公開後の定常運用を担う組織。本見積の 13 名の外。
